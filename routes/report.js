...
// SharedKeyCredential (required for SAS)
const sharedKeyCredential = new StorageSharedKeyCredential(
  process.env.AZURE_STORAGE_ACCOUNT,
  process.env.AZURE_STORAGE_ACCOUNT_KEY
);

// Helper: generate SAS URL (valid for 1 hour)
...
const uploadToAzure = async (file) => {
  try {
    ...
    // Return SAS URL instead of raw blob URL
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (type === "SOS") {
        ...
        return res
          .status(400)
          .json({ error: "Destination required for Booking" });
      }
